/**
 * WebSocket client for federated server real-time communication
 */

import { BaseWebSocket, type BaseMessageHandler } from "./baseWebSocket";

export type MessageHandler = BaseMessageHandler<ServerMessage>;
...
export interface WsEncryptedMessage {
    id: string;
    channel_id: string;
    sender_id: string;
...
export class FederatedWsClient extends BaseWebSocket<ServerMessage, ClientMessage> {
    private domain: string;
    private sessionToken: string;
    private subscribedChannels: Set<string> = new Set();
    // private isConnecting = false; // Handled by BaseWebSocket check
    ...
    protected getPingMessage(): ClientMessage | null {
        return { type: 'ping' };
    }
