...
import type { DecryptedMessage, DmPreview, SidebarView, SystemMessageType } from "../../types/index";
...
export function useGroupOpen({
  ...
  const openGroupFromPreview = useCallback(async (preview: DmPreview) => {
    ...
    try {
      ...
      for (const msg of msgs) {
        ...
        if (isSystemMessage) {
          const actorName = msg.sender_id === userId ? "You" : (nameMap.get(msg.sender_id) || "Someone");
          let content = "";
          try {
            ...
            if (msg.message_type === "group_member_added" && Array.isArray(payload.added_user_ids)) {
              ...
              const name = payload.user_id === userId ? "You" : (nameMap.get(payload.user_id) || "Someone");
  ...
  }, [userId, userKeys, setSidebarView, setIsLoadingChat, setChatMessages, setActiveChat, setUnreadCounts, setDmPreviews, setError]);
