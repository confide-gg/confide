...
export function useChatWebSocket({
  ...
  useEffect(() => {
    ...
    const unsubscribe = centralWebSocketService.onMessage((message) => {
      switch (message.type) {
        ...
        case "message_deleted":
          ...
          break;
        case "message_edited":
          if (activeChat && activeChat.conversationId === message.data.conversation_id && userKeys) {
            (async () => {
              try {
                ...
                if (parsedMsg.chain_id !== undefined && parsedMsg.iteration !== undefined) {
                  if (keyResponse.encrypted_key) {
                    const senderKeyState = await cryptoService.decryptFromSender(userKeys.kem_secret_key, keyResponse.encrypted_key);
        ...
        case "typing":
          {
            ...
            if (activeChat && activeChat.conversationId === conversation_id && user_id !== userId && user_id) {
              if (is_typing) {
                setTypingUsers(prev => {
    ...
    return () => unsubscribe();
  }, [userId, userKeys, activeChat, friendsLogic, chatLogic, loadGroupPreviews, setSuccessMessage, setTypingUsers]);
}
