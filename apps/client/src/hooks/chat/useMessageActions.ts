...
interface UseMessageActionsParams {
  ...
  editMessageMutation: {
    mutateAsync: (params: { messageId: string; data: EditMessageRequest }) => Promise<unknown>;
  };
  addReactionMutation: {
    mutateAsync: (params: { messageId: string; emoji: string }) => Promise<unknown>;
  };
...
export function useMessageActions({
    ...
    async (messageId: string, newContent: string) => {
      ...
      try {
        ...
        if (activeChat.isGroup) {
          ...
          encrypted = cryptoService.stringToBytes(
            JSON.stringify({ ciphertext: Array.from(encryptedBytes) })
          );
  ...
  const addSystemMessage = useCallback(
    (conversationId: string, content: string, systemType: SystemMessageType) => {
