...
export function useIncomingMessages({
    ...
    const handleIncomingMessage = useCallback(async (msgData: IncomingMessageData) => {
        ...
        if (isSystemMessage) {
            ...
            if (currentChat?.isGroup) {
                ...
                const actorName = msgData.sender_id === userId ? "You" : (nameMap.get(msgData.sender_id) || "Someone");
                try {
    ...
    }, [userId, userKeys, friendsList, profileStatus, activeChatRef, processedMessageIds, groupMemberNameCache, setUnreadCounts, setDmPreviews, setChatMessages, queryClient]);
