...
export function useChatMessages(friendsList: Friend[]) {
  ...
  const { loadFavoriteGifs, toggleFavoriteGif } = useGifFavorites(
    favoriteGifUrls,
    setFavoriteGifUrls
  );
  ...
  const { deleteMessage, pinMessage, unpinMessage, editMessage, addReaction, addSystemMessage } =
    useMessageActions({
      activeChat,
      userKeys,
      userId: user?.id,
