...
export function CallView({ onMinimize, isMinimized: _isMinimized = false }: CallViewProps) {
  ...
  const [screenShareHeight, setScreenShareHeight] = useState(60);
  const [isDraggingScreenShare, setIsDraggingScreenShare] = useState(false);
  ...
  const handleScreenShareDragStart = useCallback((e: React.MouseEvent) => {
    e.preventDefault();
    setIsDraggingScreenShare(true);
    dragStartRef.current = { y: e.clientY, height: screenShareHeight };
  }, [screenShareHeight]);

  useEffect(() => {
    if (!isDraggingScreenShare) return;
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const newHeight = Math.min(85, Math.max(30, dragStartRef.current.height + deltaPercent));
      setScreenShareHeight(newHeight);
    };

    const handleMouseUp = () => {
      setIsDraggingScreenShare(false);
      ...
      <div className="flex-1 flex flex-col relative overflow-hidden min-h-0">
        {hasScreenShare ? (
          <ScreenShareLayout
            ...
            onDragStart={handleScreenShareDragStart}
            isDragging={isDraggingScreenShare}
          />
        ) : (
          <DefaultCallLayout
            myName={myDisplayName}
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      <div
        ...
        style={{ height: `${screenShareHeight}%` }}
      >
        {peerIsSharing ? (
      ...
      <div
        className={cn(
          "h-2.5 bg-background flex items-center justify-center cursor-ns-resize group border-y border-border flex-shrink-0",
          isDragging && "bg-white/10"
        )}
        onMouseDown={onDragStart}
      >
        <GripHorizontal className={cn(
          "w-5 h-3 text-white/30 group-hover:text-white/60 transition-colors",
          isDragging && "text-white/60"
        )} />
      </div>

      <div className="flex-1 bg-background p-3 overflow-y-auto min-h-0">
