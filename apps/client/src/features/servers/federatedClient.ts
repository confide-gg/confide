...
export class FederatedServerClient {
  ...
  private async fetch<T>(path: string, options: RequestInit = {}): Promise<T> {
    ...
    if (!response.ok) {
      ...
      console.error(
        `[FederatedClient] Request to ${path} failed with status ${response.status}: ${text}`
      );
      const err = new Error(`Federated request failed: ${text}`) as Error & {
        status?: number;
        path?: string;
      };
  ...
  public async updateRole(
    roleId: string,
    data: { name?: string; permissions?: number; color?: string; position?: number }
  ): Promise<any> {
